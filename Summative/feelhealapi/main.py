...
class HealthDataInput(BaseModel):
    ...
    daily_steps: int = Field(
        ..., 
        ge=0,
        description="Number of steps walked per day",
        example=8000
    )
    calories_burned: float = Field(
        ..., 
        ge=0,
        description="Total calories burned per day",
        example=2200.5
    )
    ...
    medication_usage: int = Field(
        ...
        ge=0, 
        le=1,
        description="Medication usage (0: Yes, 1: No)",
        example=1
    )
...
class PredictionResponse(BaseModel):
    ...
    anxiety_confidence: float = Field(..., description="Confidence score for anxiety prediction")
    ...
    depression_confidence: float = Field(..., description="Confidence score for depression prediction")
    contributing_factors: List[str] = Field(..., description="List of identified risk factors")
...
def custom_openapi():
    ...
    openapi_schema = get_openapi(
        ...
        ## Overview
        The API analyzes various health metrics to predict potential mental health risks and provides:
        * Anxiety and depression risk levels
        ...
        * Contributing risk factors
        ...
        ## Error Handling
        The API uses standard HTTP status codes and provides detailed error messages when something goes wrong.
        """,
        routes=app.routes,
    )
...
async def predict_mental_health(data: HealthDataInput):
    ...
    try:
        ...
        # Scale the features
        scaled_data = model_info['scaler'].transform(
            df[model_info['feature_columns']]
        )
